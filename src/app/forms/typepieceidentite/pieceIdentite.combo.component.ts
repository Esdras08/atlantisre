...
@Component({
    selector: 'app-combo-re-identite',
    template: `
...
export class PieceIdentiteComboComponent extends SgiCombo {
    ...
    beforeAll(): void {
        ...
        const model = new PieceIdentiteModel();
        this.setModel(model);
        this.getRequest().setMethod(RequestMethod.POST)
            .setUrl(WebServicesUtilities.getSimpleUrl2(ERP.UrlControlers.Generated, API.TYPE_PIECE_IDENTITE.GET))
            .setData({
                TakeAll: true,
                SortOrder: 'Ascending',
                ItemToSearch: {
