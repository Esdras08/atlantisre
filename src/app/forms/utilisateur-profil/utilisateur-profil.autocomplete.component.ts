...
import {UtilisateurProfilModel} from '../../core/models/utilisateur-profil.model';

@Component({
    selector: 'app-autocomplete-utilisateur-profil',
    template: `
...
export class UtilisateurProfilAutocompleteComponent extends SgiAutocomplete {
    ...
    beforeAll(): void {
        const model = new UtilisateurProfilModel();
        this.setModel(model);
        this.setSearchParams([
            new ModelMaskFieldImpl<any>().setTitle('IdUtilisateurFonctionnalite').setName('IdUtilisateurFonctionnalite'),
        ]);
        this.setDisplayedField('IdUtilisateurFonctionnalite');

        this.getRequest().setMethod(RequestMethod.POST)
            .setUrl(WebServicesUtilities.getSimpleUrl2(ERP.UrlControlers.Generated, API.UTILISATEUR_PROFIL.GET))
            .setData({
                Index: 0,
                Size: 5,
                TakeAll: false,
