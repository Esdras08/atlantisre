...
@Component({
    selector: 'app-combo-re-juridique',
    template: `
...
export class FormeJuridiqueComboComponent extends SgiCombo {
    ...
    beforeAll(): void {
        ...
        const model = new FormeJuridiqueModel();
        this.setModel(model);
        this.getRequest().setMethod(RequestMethod.POST)
            .setUrl(WebServicesUtilities.getSimpleUrl2(ERP.UrlControlers.Generated, API.FORME_JURIDIQUE.GET))
            .setData({
                TakeAll: true,
                SortOrder: 'Ascending',
                ItemToSearch: {
