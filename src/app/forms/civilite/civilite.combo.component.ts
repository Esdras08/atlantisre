...
@Component({
    selector: 'app-combo-re-civilite',
    template: `
...
export class CiviliteComboComponent extends SgiCombo {
    ...
    beforeAll(): void {
        ...
        const model = new CiviliteModel();
        this.setModel(model);
        this.getRequest().setMethod(RequestMethod.POST)
            .setUrl(WebServicesUtilities.getSimpleUrl2(ERP.UrlControlers.Generated, API.CIVILITE.GET))
            .setData({
                TakeAll: true,
                SortOrder: 'Ascending',
                ItemToSearch: {
